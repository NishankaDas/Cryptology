...
// Generic permutation
uint64_t permute(uint64_t in, int *table, int n, int inBits) {
    uint64_t out = 0;
    for (int i = 0; i < n; i++) {
        out <<= 1;
...
uint32_t feistel(uint32_t R, uint64_t key) {
    uint64_t expanded = permute((uint64_t)R << 32, E, 48, 64);
    expanded ^= key;

    uint32_t output = 0;

    for (int i = 0; i < 8; i++) {
        int block = (expanded >> (42 - i*6)) & 0x3F;
        int row = ((block & 0x20) >> 4) | (block & 1);
        int col = (block >> 1) & 0xF;

        output = (output << 4) | S[i][row][col];
    }

    return permute((uint64_t)output << 32, P, 32, 64);
}
...
uint64_t DES(uint64_t plaintext, uint64_t key) {
    ...
    uint64_t perm = permute(plaintext, IP, 64, 64);
...
int main() {
    ...
    uint64_t cipher = DES(plaintext, key);
