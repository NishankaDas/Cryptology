...
def f_func(right, key):
    return permute(sbox_sub(xor(permute(right, E), key)), P)
...
def des_block(block, keys):
    ...
    for k in keys:
        left, right = right, xor(left, f_func(right, k))
...
def encrypt(text, key):
    ...
    for i in range(0, len(text), 8):
        block = str_to_bin(text[i:i+8])
        result += des_block(block, keys)
...
def decrypt(cipher, key):
    ...
    for i in range(0, len(cipher), 64):
        block = cipher[i:i+64]
        result += bin_to_str(des_block(block, keys))
